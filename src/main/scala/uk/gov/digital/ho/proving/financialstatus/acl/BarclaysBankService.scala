...
class BarclaysBankService extends BankService {
  ...
  def fetchAccountDailyBalances(account: Account, endDate: LocalDate, startDate: LocalDate) = {
    ...
    lazy val balances: Stream[AccountDailyBalance] = AccountDailyBalance(LocalDate.now(), BigDecimal(scala.util.Random.nextInt(2000) + 500)) #::
      balances.map { dailyBalance => AccountDailyBalance(dailyBalance.date.minusDays(1), BigDecimal(scala.util.Random.nextInt(2000) + 500)) }
